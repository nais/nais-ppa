...
resource "google_project_service" "services" {
  ...
  for i in $(seq 25); do
    sleep $i
    if gcloud services list --project="${google_project.project.project_id}" | grep ${each.value}; then
      exit 0
    fi
  done

  echo "Service was not enabled after ~5m"
  exit 1
  EOF
  }
}
...
resource "google_service_account_iam_member" "zoom-repo-update-workload-identity" {
  member             = "principalSet://iam.googleapis.com/${google_iam_workload_identity_pool.pool.name}/attribute.repository/nais/nais-ppa"
